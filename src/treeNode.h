...
nodeAST* newIdentifier      (const char* _identifier, memoryList _allocator){
    ...
    returnNode->nodeValue.identifier = (char*)mallocList(sizeof(char)*(strlen(_identifier)+20), _allocator);
    ...
    if (strcmp(_identifier, "_") == 0 || strcmp(_identifier, "main") == 0) {
        strcpy(returnNode->nodeValue.identifier, _identifier);
    } else {
        sprintf(returnNode->nodeValue.identifier, "script_%s", _identifier);
    }
    return returnNode;
}
