...
type_check * new_type_check(int size){
	...
	sym_tbl_add_entry(new_tbl_entry("script_true", 0, NULL, new_bool_type()), ret->current_tbl);
	sym_tbl_add_entry(new_tbl_entry("script_false", 0, NULL, new_bool_type()), ret->current_tbl);
	...
	if(node->nodeValue.varDeclare.type == NULL){
		...
		for(nodeAST * i = node->nodeValue.varDeclare.idList; i != NULL; i = i->nodeValue.identifierList.next){
			...
			if(current_entry != NULL){ //Redeclaration in the same scope
				...
				if(expr_type->spec_type.list_type.type_list[type_iterator]->type == INVALID_TYPE){
					sprintf(err_buf, "Invalid Assignment at line %lu \n", node->lineNumber);
					add_msg_line(err_buf, current, node->lineNumber);
					return 0;
				}
    ...
    for(nodeAST * i = node; i != NULL; i = i->nodeValue.exprList.next){
    	...
    	if(to_be_added != NULL){
			if(expr_list_type->spec_type.list_type.list_size == expr_list_type->spec_type.list_type.list_cap){
				expr_list_type->spec_type.list_type.type_list = ralloc(expr_list_type->spec_type.list_type.type_list, sizeof(type *) * expr_list_type->spec_type.list_type.list_size * 2);
			...
			i->nodeValue.exprList.expr->sym_tbl_ptr = to_be_added;
    	}
    	to_be_added = NULL;
    	}
    			...
    			*/
    				...
    				if(entry == NULL){
    					...
    					if(valid_type_conversion(entry_type, input->spec_type.list_type.type_list[0]) == 0 && input->spec_type.list_type.list_size == 1){
    						node->nodeValue.funcCall.target->sym_tbl_ptr = entry->type_info;
    						node->nodeType = EXPR_CAST;
    						return entry->type_info;
    					}else{
...
type * type_check_expr_and(nodeAST * node, sym_tbl * scope){
							type * l = (type_check_expr(node->nodeValue.logicAnd.left, scope));
							type * r = (type_check_expr(node->nodeValue.logicAnd.right, scope));
    						type * left =  get_alias_type(l);
    						type * right = get_alias_type(r);
    						//Both must be boolean for and
    						if(valid_type_comparison(l, r) == 0 && (left->type == LITERAL_BOOL && right->type == LITERAL_BOOL) ){
...
type * type_check_expr_or(nodeAST * node, sym_tbl * scope){
							type * l = (type_check_expr(node->nodeValue.logicOr.left, scope));
							type * r = (type_check_expr(node->nodeValue.logicOr.right, scope));
    						type * left =  get_alias_type(l);
    						type * right = get_alias_type(r);
    						//Both must be boolean for and
    						if(valid_type_comparison(l, r) == 0 && (left->type == LITERAL_BOOL && right->type == LITERAL_BOOL) ){
...
type * type_check_expr_bitAndNot(nodeAST * node, sym_tbl * scope){
    						type * left = (type_check_expr(node->nodeValue.bitAndNot.left, scope));
    						type * right = (type_check_expr(node->nodeValue.bitAndNot.right, scope));
    						if(valid_type_comparison(left, right) == 0 && (left->type == LITERAL_INT || left->type == LITERAL_RUNE || left->type == ALIAS_TYPE )){
    							node->sym_tbl_ptr = new_int_type();
    							return node->sym_tbl_ptr;
    						}
...
type * type_check_expr_unary_pos(nodeAST * node, sym_tbl * scope){
    						...
    						type * actual = (type_check_expr(node->nodeValue.pos.expr, scope));
    						type * left = get_alias_type(actual);
    						if( left->type == LITERAL_INT || left->type == LITERAL_FLOAT || left->type == LITERAL_RUNE ){
    							node->sym_tbl_ptr = actual;
    							return (actual);
    						}
    						else{
    							print_type_to_string(left, type_buf);
...
type * type_check_expr_unary_neg(nodeAST * node, sym_tbl * scope){
    						//Characters are not allowed as part of unary expression
    						type * actual = (type_check_expr(node->nodeValue.neg.expr, scope));
    						type * left = get_alias_type(actual);
    						if( left->type == LITERAL_INT || left->type == LITERAL_FLOAT || left->type == LITERAL_RUNE ){
    							node->sym_tbl_ptr = actual;
    							return (actual);
...
type * type_check_expr_eq(nodeAST * node, sym_tbl * scope){
    						...
    						if(valid_type_comparison(left, right) == -1 || valid_equal(left) == -1){
    								print_type_to_string(left, type_buf);
    								print_type_to_string(right, type_buf_extra);
...
type * type_check_expr(nodeAST * node, sym_tbl * scope){
    		...
    		case EXPR_BINARY_OP_ANDNOT:
    						ret = type_check_expr_bitAndNot(node, scope); break;
...
*/
                    	...
                    	case STATE_ASSIGN_ADD: {
                    							...
                    							if(valid_type_comparison(left, right) == -1 || (left->type != LITERAL_INT && left->type != LITERAL_RUNE && left->type != LITERAL_FLOAT && left->type != ALIAS_TYPE)){
                    								print_type_to_string(left, type_buf);
      												print_type_to_string(right, type_buf_extra);
													sprintf(err_buf, "Invalid type. Left Argument is of %s. Right is currently of %s at line %zd",type_buf , type_buf_extra,node->lineNumber);
													add_msg_line(err_buf, current, node->lineNumber);
                    	...
                    	case STATE_ASSIGN_MOD: {
                    							...
                    							if(valid_type_comparison(left, right) == -1 || (left->type != LITERAL_INT && left->type != LITERAL_RUNE && left->type != ALIAS_TYPE)){
                    								print_type_to_string(left, type_buf);
      												print_type_to_string(right, type_buf_extra);
                    	...
                    	case STATE_ASSIGN_SHIFTLEFT: {
                    							type * left = type_check_expr(node->nodeValue.assignShiftLeft.left, scope);
                    							type * right = type_check_expr(node->nodeValue.assignShiftLeft.right, scope);
                    							if(valid_type_comparison(left, right) == -1 || (left->type != LITERAL_INT && left->type != LITERAL_RUNE && left->type != ALIAS_TYPE) ){
                    								print_type_to_string(left, type_buf);
      												print_type_to_string(right, type_buf_extra);
                    	...
                    	case STATE_ASSIGN_SHIFTRIGHT: {
                    							type * left = type_check_expr(node->nodeValue.assignShiftRight.left, scope);
                    							type * right = type_check_expr(node->nodeValue.assignShiftRight.right, scope);
                    							if(valid_type_comparison(left, right) == -1 || (left->type != LITERAL_INT && left->type != LITERAL_RUNE && left->type != ALIAS_TYPE)){
                    								print_type_to_string(left, type_buf);
      												print_type_to_string(right, type_buf_extra);
                    	...
                    	case STATE_ASSIGN_ANDNOT: {
                    							type * left = type_check_expr(node->nodeValue.assignAndNot.left, scope);
                    							type * right = type_check_expr(node->nodeValue.assignAndNot.right, scope);
                    							if(valid_type_comparison(left, right) == -1 || (left->type != LITERAL_INT && left->type != LITERAL_RUNE && left->type != ALIAS_TYPE) ){
                    								print_type_to_string(left, type_buf);
      												print_type_to_string(right, type_buf_extra);
													sprintf(err_buf, "Must be boolean type. Left Argument is of %s. Right is %s at line %zd",type_buf , type_buf_extra,node->lineNumber);
...
*/
	...
	if(switch_cond->nodeValue.switchCondition.expr != NULL){
		...
		if(switch_expr->type == INVALID_TYPE){
       		sprintf(err_buf, "Switch statment condition invalid at line %zd",node->lineNumber);
			add_msg_line(err_buf, current, node->lineNumber);
		}
	}else{
