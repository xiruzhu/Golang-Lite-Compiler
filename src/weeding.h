...
bool weedingFunctionReturned(nodeAST* _funcNode) {
    ...
    switch (_funcNode->nodeType) {
        ...
        case STATE_UTILITY_CASE_LIST: {
            ...
            while (ptr != NULL) {
                if (ptr->nodeValue.caseList.state->nodeValue.caseClause.expr == NULL) has_default = true;
...
bool weedingAssignableExpr(nodeAST* _AST) {
    ...
    switch (_AST->nodeType) {
        ...
        case EXPR_INDEX:    return weedingAssignableExpr(_AST->nodeValue.index.target);     // id[index] = expr
        case EXPR_SELECTOR: return weedingAssignableExpr(_AST->nodeValue.selector.target);  // id.entry  = expr
        case EXPR_SLICE:    return weedingAssignableExpr(_AST->nodeValue.slice.target);     // id[]      = expr
        default:            return false; //
    }
    return false;
}
...
void weedingInit(nodeAST* _program) {
    ...
    switch (_program->nodeType) {
        case PROG_PACKAGE: {
            nodeAST* package_name = _program->nodeValue.package.identifier;
            if (strcmp(package_name->nodeValue.identifier, "_") == 0) {
                dumpErrorMsg("package name must not be the blank identifier", _program->lineNumber);
            }
            return;
        }
        case PROG_PROGRAM: {
            weedingInit(_program->nodeValue.program.package);
            weedingInit(_program->nodeValue.program.program);
            return;
        }
        case PROG_LIST: {
