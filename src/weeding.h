...
void weedingInit(nodeAST* _program) {
    ...
    case PROG_PACKAGE: {
    if (strcmp(_program->nodeValue.package.identifier->nodeValue.identifier, "_") == 0L) {
	dumpErrorMsg("_ is not a valid pacakge name", _program->nodeValue.package.identifier->lineNumber);
	return;
    }
    return;
    }
