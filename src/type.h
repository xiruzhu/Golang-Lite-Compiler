...
int valid_equal(type * arg0){
     switch(arg0->type){
        ...
        case LITERAL_STRING:
                return 0;
        case ALIAS_TYPE:
            return valid_equal(arg0->spec_type.alias_type.a_type);
        case ARRAY_TYPE:
            return valid_equal(arg0->spec_type.array_type.a_type);
        case STRUCT_TYPE:
            for(int i = 0; i < arg0->spec_type.struct_type.list_size; i++){
                if(valid_equal(arg0->spec_type.struct_type.type_list[i]) == -1)
                    return -1;
            }
            return 0;
        default: return -1;
